...
//EFFECTS: append each element of list2 to list1 one by one
{
    if (list_isEmpty(list2))
        ...
        return foldL(my_list_make(list1, list2), list_rest(list2));
}
...
//EFFECTS: append each element of list2 that satisfies 'judge' to list1
{
    if (list_isEmpty(list2))
        ...
        return foldL(my_list_make(list1, list2, judge), list_rest(list2), judge);
}
...
//EFFECTS: append the first n elements of list2 to list1 one by one
{
    if (n <= 0)
        ...
        return foldL(my_list_make(list1, list2), list_rest(list2), n - 1);
}
...
//EFFECTS: identical with tree_hasPathSum except that it considers the path sum of an empty tree is zero
{
    ...
    if (!hasLeft && !hasRight)
        ...
        return my_tree_hasPathSum(right, sum - elt);
    else if (hasLeft && !hasRight)
        return my_tree_hasPathSum(left, sum - elt);
    else
        return my_tree_hasPathSum(left, sum - elt) || my_tree_hasPathSum(right, sum - elt);
}
